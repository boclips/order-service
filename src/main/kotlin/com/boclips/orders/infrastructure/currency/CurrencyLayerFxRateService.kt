...
class CurrencyLayerFxRateService(private val accessKey: String) : FxRateService {

    private val restTemplate = RestTemplate()

    override fun getRate(from: Currency, to: Currency, on: LocalDate): BigDecimal {
        val endpoint = "http://apilayer.net/api/historical?access_key={accessKey}&date={date}&source={from}&currencies={to}"

        val response = restTemplate.getForObject(endpoint, CurrencyLayerResponse::class.java, mapOf(
